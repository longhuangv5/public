...
!function () {
    ...
    app.config(['$httpProvider', function ($httpProvider) {
    ...
    }]).factory("http", ['$http', 'pop', '$document', '$location', 'urlFileName', 'parkId','$window',function ($http, pop, $document, $location, urlFileName, parkId,$window) {
        var needProgress = false,getProgressStatus = false;
        ...
        function send(method, url, params, data, callback, fail) {
            if(!getProgressStatus && !needProgress && $window.currSys === 'subSystem'){
                needProgress = true;
            }
            getProgressStatus = true;
            if (!params.parkId && !data.parkId) params.parkId = parkId ? parkId : 1;
            needProgress && pop.wait();
            $http({
                method: method,
                url: getPath() + url,
                params: params,
        ...
        function serviceError(res) {
            if(res.goLogin){
                var addr = res.loginUrl.slice(1)+'.jsp';
                angular.isFunction(parent.goLogin) ? (parent.goLogin(getPath()+addr)) : (window.location.href = getPath()+addr);
            }else{
        ...
        // function serviceError(res) {
        //     if(res.goLogin){
        //         var addr = res.loginUrl;
        //         angular.isFunction(parent.goLogin) ? (parent.goLogin(addr)) : (window.location.href = addr);
        //     }else{
        //         pop.toast('错误提示', res.execMessage, pop.types.warning);
        //     }
        // }
